...
| `deck_slots` | Deck-slot vocabulary resolver: `canonicalize_slot_args(...)` rewrites a liquid handler's slot arguments (`tips.reset`/`tips.mark` `slot`, `move_labware` `new_location`, `setup` `labware[].location`, `deck.declare` keys) from any spelling of a place — registry name `ot2_hte/slot_2`, `"slot 2"`, an xArm node id — to the device's bare key, refusing another device's place (`SlotResolutionError`). Runs inside `validate_plan` + `execute_plan`; `location_vocabulary(...)` is the map for agents. Reads `LabSession.locations` (default: `locations.yaml`, lazily). |

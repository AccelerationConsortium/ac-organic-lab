...
| `workflow.py` | Authorized-run executor (Phase F): pulls a bitácora run authorization, re-verifies everything, drives the pinned package through `execute_plan` with an SSE step stream and cooperative abort; `plan_run` audit rows; D-23 filing to AnaliticaDB via `record.py`. |
...
| `record.py` | D-23 record layer: files finished runs in AnaliticaDB (Experiment ensured, run as `Plan`, non-success steps as `Note`s). |

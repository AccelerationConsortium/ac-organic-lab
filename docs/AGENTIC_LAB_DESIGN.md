...
| `hostops_lle_pc` | `sdl2-pc-00-lle` (the Process Chemistry / LLE PC, `SDL2Win02`; NSSM service `sdl-lab-hostops`, :8060, LocalSystem) | streamable-http + bearer token | **empty — read-only until the platform's STATUS_SPEC servers exist there** (the Agilent ChemStation Data Service and OpenLAB license services are present but deliberately not whitelisted) | **2026-09-06** — installed from gaia after the lab-ops key grant; verified: tokenless `initialize` → 401, `host_info` / `list_serial_ports` answer, a non-whitelisted service is refused |
| `hostops_pi0_lle` | `sdl2-pi0-lle-pizerocam` (Pi Zero 2W, PiZeroCam pH/camera unit; `~caoyang/sdl-lab-hostops`) | daemonless stdio-over-SSH (`lle-pi` alias, user `caoyang`) | **empty — nothing runs as a unit there yet** (PiZeroCam is a project; `use_sudo = true` is armed for when it does) | **2026-09-06** — venv via pip (no uv on the Pi); handshake + `host_info` + `list_serial_ports` verified from gaia over SSH |

...
| `hostops_gibbie_pc` | `sdl2-pc-04` (the Gibbie PC; NSSM service `sdl-lab-hostops`, :8060 — planned) | streamable-http + bearer token | planned: `gibbie-server` (the read-only bench monitor; restarting it touches no hardware) | **registered 2026-09-06, not yet deployed**: the PC accepts no lab-ops key yet (DEVICE_PC_SETUP §2.4 grant pending), so its slot on the PCs & Servers page reads unreachable until the install lands |

...
service runs on the UPLC PC, which has a wired route to the arm; there is no
...
2. **A read-only deploy key**, scoped to the one repo. Restores `git pull`;
   ...
   SSH remote, and one key per private repo. **Chosen and in place on
   `sdl2-pc-00-lle` for `mt-easymax-server` (2026-09-09) and
   `mt-xpr-balance-server` (2026-09-10), and on `sdl2-pc-04` for
   `mt-xpr-balance-server` (2026-09-10, replacing its tree copy) — recipe
   below.**
...
interfere (the LLE PC now holds one key each for `mt-easymax-server` and
`mt-xpr-balance-server`). Two more traps from driving that PC over SSH: run
PowerShell as `powershell -File <copied .ps1>`, not streamed on stdin — the
first child process that inherits stdin (`ssh-keygen`, `git`) swallows the
rest of the script; and keep the file ASCII-only, since PowerShell 5.1 reads a
BOM-less file as cp1252 and an em-dash inside a string literal is a parse
error that stops the whole file from running.
...
| `sdl-lab-hostops`         | `sdl-lab-hostops` | 8060 | `run --extra serial lab-hostops-serve --transport http` — whitelisted host-ops MCP server (service status/logs/restart, serial enumeration, local `/status` probes) consumed by the central agent; see [`AGENTIC_LAB_DESIGN.md`](AGENTIC_LAB_DESIGN.md). **Documented §5 exception:** runs as `LocalSystem` — it needs service-control rights over its NSSM neighbours and touches no vendor `HKCU` profile or COM port, so neither reason behind §5 applies. Requires `HOSTOPS_TOKEN` in the service env (non-loopback bind refuses to start without it). | `sdl2-pc-03-cytation.<tailnet>` (deployed + verified 2026-08-11); `sdl2-pc-06-uplc` (2026-08-11); `sdl2-pc-04` (Gibbie PC, 2026-09-06 — whitelist `gibbie-server`, `--link-mode copy` + `UV_LINK_MODE=copy` like that PC's monitor, firewall rule `sdl-lab-hostops 8060`; `mt-xpr-balance` added to whitelist + restartable and 8081 to the probe ports 2026-09-10); `sdl2-pc-00-lle` (LLE PC, 2026-09-06 — whitelist `mt-easymax` since it was deployed the same day, a safe restart because the service holds no instrument state; `C:\SDL_Tools` created that day with `uv.exe` copied from the user profile and `nssm.exe` copied byte-for-byte from the Gibbie PC; `mt-xpr-balance` added to whitelist + restartable and 8081 to the probe ports 2026-09-10) |
| `sdl2-gibbie-server`      | `gibbie-server` | 8070 | `run --project C:\Users\sdl2\Projects\sdl2-gibbie-server --link-mode copy gibbie-server --config <dir>\config.toml` — read-only STATUS_SPEC gateway for the Gibbie sample-prep bench (UR arm, XPR balance, Flex, hotplate, the Gibbie workflow's UI bridge); one envelope per device at `/devices/{id}/status`, no `/control/*`. **§5 exception:** runs as `LocalSystem` (touches no vendor `HKCU` profile and opens no COM port — it only enumerates them), so its `~` is the SYSTEM profile: the Flex SSH probe needs `ssh_host` / `ssh_key_file` spelled out in `config.toml` and `GIBBIE_FLEX_SSH_KEY_PASSPHRASE` in the service env. Synced with `--link-mode copy` (service env `UV_LINK_MODE=copy`) because the only shell on that PC is an elevated SSH session. | `sdl2-pc-04.<tailnet>` (deployed + verified 2026-09-06) |
| `mt-easymax-server`       | `mt-easymax`   | 8082 | `run --project C:\Users\sdl2\Projects\mt-easymax-server --link-mode copy --extra opcua mt-easymax-serve --config <dir>\config.toml` — STATUS_SPEC v1.2 control service for the Process Chemistry EasyMax, through Mettler's **Reactor Device Server** on this PC's loopback (OPC UA :50008). Two things that will otherwise cost an afternoon: the RDS is a **.NET console application, not a Windows service**, so if it is not running this service reports `unknown` and there is nothing wrong with the instrument; and with the *instrument* powered off the RDS still answers and still serves the identity it cached, while every live parameter returns `BadNoDataAvailable` — also `unknown`, also not a fault. `EASYMAX_PASSWORD` comes from the RDS `appsettings.json` `operator` account and lives in the service env. Deployed + verified 2026-09-06. | `sdl2-pc-00-lle.<tailnet>` |
...
| `dobot-MG400-server`      | `dobot-mg400`  | 8050 | `run --extra api dobot-mg400-serve` (serves `/status`, `/control/*`, `/graph`, `/ws`, and the operator panel at both `/ui/` and `/web/`) | `sdl2-pc-05-dobot.<tailnet>` (`100.64.254.18`) — Prototyping PC |
| `robot-motion` prerelease | `robot-motion-prototype` | 8075 | Separate checkout/environment, bundled `/web/` UI, read-only UR Dashboard observation every 15 seconds and offline graph preview. No physical control endpoints. Selected extra `ur`; existing xArm compatibility application unchanged, MG400 driver planned. On the UPLC PC, an isolated 64-bit managed Python runtime lives under the checkout's ignored `.state/python`; existing Python/Anaconda installations and chromatography services are untouched. Automatic startup, crash restart, online 10 MiB log rotation under `.state/logs` (isolated prototype exception to the shared log directory). **§5 exception:** LocalSystem for read-only network observation only; future control requires a separate reviewed deployment. Bind and firewall permit only Tailnet access; no public edge route. | UPLC PC; installed and verified, dashboard ID `ligand_ur5e`. |

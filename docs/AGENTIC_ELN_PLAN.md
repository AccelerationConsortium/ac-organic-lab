...
- **Vocabulary is pinned** (do not conflate): **protocol** (git template) ≠
  **Plan** (rendered, versioned run record in BitacoraDB) ≠ **workflow**
...
### Step 2 — Experiment-plan-as-data + design surface — *LaAgenteAnalitica + BitacoraDB (wiring only)*
...
`validate_plan`) + `register_plan` writing the BitacoraDB `Plan` (draft).
...
### Step 5 — Step-anchored Notes — *LaAgenteAnalitica + BitacoraDB (Note API)*
...
5. **Phase E — run authorizer + compiler (dry-run first).** Authorization pins +
   ...
   **fails** compilation; the package digest covers `design` and `plate_map`, so
   ...
   BitacoraDB contract bump for run-authorization linkage
...
6. **Phase F — authorized execution.** Wire run authorizations into the
   ...
   **Proven live 2026-08-14:** the first real authorized run —
   `ra_67f32cb0920b4a41`, the 14-step `ot2-transfer-smoke` on
   `ot2_complexation` — executed end-to-end: human authorization in
   bitácora → Slack trigger by the boxed `lab-runner` agent (`lab-runs`
   MCP) → executor with per-step claims and between-step revocation
   re-checks → all 14 steps ok on hardware in 146 s → record filed in
   BitacoraDB (D-23). A first attempt the same evening failed on a campus
   network outage at step 1 and filed as `abandoned` with the claim
   released — fail-fast and the audit trail behaving as designed.
...
| **D-7** | **Protocol schema growth ownership — RESOLVED (2026-07-22):** `bitacora` hosts multiple templates (one per experiment type), and the **HTE template** is one of them — it contains the core structural blocks (design matrix, plate map, step list, QC, materials). Other templates (synthesis, characterization, etc.) can be added alongside it later. Per-project chemistry extensions stay in the project repo (chemistry-specific vocabulary, validated alongside the core template schema in CI). **Template repo absorbed (Option B):** the existing `organic-hte-template` repo is folded into `bitacora` as `bitacora/templates/hte/` (skeleton + schema together); new projects are stamped from `bitacora` directly; no separate template repo. `pins.yaml` in each project repo pins the bitacora template version it conforms to (same cross-repo pin pattern as the BitacoraDB ontology pin). **Landed (2026-07-22):** template absorbed as `bitacora/templates/hte/` at `TEMPLATE_VERSION` 1.2.0 (pin renamed `organic-hte-template` → `bitacora-hte-template`; see template `CHANGELOG.md`); `bitacora` commit `e5374f2`. | Core structural blocks in `bitacora/templates/hte/`; chemistry-specific vocabularies per-project; `organic-hte-template` absorbed into `bitacora` | Phase B |
...
| **D-15** | **DB: report generation location** — generate in the agent repo, store in BitacoraDB (assumed), or generate server-side. | Keep generate-there, store-here | Track 1 Step 6 |
| **D-16** | **Conversation-store engine + hosting — RESOLVED (2026-07-22):** greenfield Postgres JSONB, in a separate `conversations` database on the BitacoraDB Postgres instance, as a thin persistence module inside `bitacora`. One engine to operate (same as BitacoraDB), contract untouched, persistence boundary (§13) preserved. Not Mongo (no GraphChat to reuse — `bitacora` is its own platform), not inside the BitacoraDB database/contract (would weld a redactable chat log onto an immutable record, force `SCHEMA_VERSION` bumps, double writes via `agent_actions`), not a second BitacoraDB-style service (one writer = the agent backend persisting its own AG-UI stream; read surface = the chat UI's room history; no ontology export, no audit table, no public API until a concrete reader needs one). See design §13 for the three-tier record (conversation store / `decisions.md` / prose-free BitacoraDB). | — | — |
...
| **D-23** | **Where the run record goes — RESOLVED (2026-08-08): BitacoraDB, per the mapping in [`DATABASE_DESIGN.md`](DATABASE_DESIGN.md) §"ELN artifacts → record layer".** A run is a `Plan` row under the campaign's `Experiment`; deviations and device faults are `step_id`-anchored `Note`s, so "where did this run go wrong" is a query. Not the dashboard's `lab.db`, which holds lab telemetry, not experiment data. **This is not blocked on an ontology bump** — contrary to the earlier reading, BitacoraDB is live at `SCHEMA_VERSION 0.11.0` with `/plans`, `/notes`, `/experiments` and every field required (`PlanCreate.source_commit`/`protocol_path`/`steps`, `NoteCreate.step_id`/`kind`/`corrects`, `ExperimentCreate.operator`); what is missing is the cross-repo wiring. ~~The first Phase F slice still writes no records~~ — **wiring shipped 2026-08-13** (`api/app/record.py`, invoked from `workflow.py` after every finished run): Experiment ensured from the design slug (protocol path as fallback), `Plan` + per-non-success-step `Note`s POSTed with the shared edge secret, executor note kinds translated at the boundary to BitacoraDB's `NoteKind` enum (`device_fault`→`deviation`, `outcome_unknown`→`event`, the reported kind preserved in `data.kind_reported`). A record-write failure never fails the run; unconfigured deployments report `not_configured`. Production env configured the same day. **First real run filed 2026-08-14** (`ra_67f32cb0920b4a41` → Plan `completed` under experiment `ot2-transfer-smoke`, launched by `hermes@lab.local`, authorized by a human), after two write-path fixes the first filing surfaced: the recorder now forwards `X-Auth-Projects` (BitacoraDB's `can_read` row filter otherwise blinds the ensure-read to the existing Experiment — the §2.1 trap, commit `5d064c4`) and walks `draft → approved → executing → completed` because no direct `draft → completed` edge exists, stamping the `approved` edge with `meta.authorized_by`, never the launcher (commit `e5bb24c`). The failed transport attempt earlier the same evening filed as `abandoned` — the audit trail working. | Records in BitacoraDB — **implemented, configured, and proven by the first real run** | Phase F |

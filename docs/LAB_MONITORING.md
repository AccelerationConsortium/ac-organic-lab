...
| `hostops_action` | **Device-pushed** by `sdl-lab-hostops` instances (first: `hostops_cytation_pc`, 2026-08-11) | One audit row per **mutating** host-ops tool call — today that is only `restart_service` (reads are deliberately not audited). `payload.extra: {action: "restart_service", service, ok, source: "lab-hostops"}`; `device_id` is the hostops instance's `equipment_id`, the target service rides in `extra.service`. Best-effort per the exporter contract above; enabled by `HOSTOPS_INGEST_URL` (base URL — the path is appended). |
...
| `hostops_action` | **Host-pushed** by `sdl-lab-hostops` instances ([AccelerationConsortium/sdl-lab-hostops](https://github.com/AccelerationConsortium/sdl-lab-hostops), registered in `mcp/servers.yaml`) | One audit row per **mutating** host-ops tool call — today only `restart_service`: `message: "restart_service <svc>: ok\|FAILED"`, `payload.extra: {action, service, ok, source: "lab-hostops"}`. `device_id` is the instance's `equipment_id` (e.g. `hostops_cytation_pc`) — the *host*, not the device whose service was restarted. Reads (status/log-tail/probes) are deliberately not audited. Best-effort: enabled by `HOSTOPS_INGEST_URL`, and a failed post never blocks the restart. See [`AGENT_OPS.md`](AGENT_OPS.md). |

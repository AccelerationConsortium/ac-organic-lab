...
| `realsense_capture` | *reserved — not yet emitted* (planned: xArm exporter, on `POST /control/realsense/capture`) | The xArm's eye-in-hand D435i writes a durable capture record (colour + 16-bit depth + `meta.json`) under `C:\SDL_Data\xarm\realsense`, but the capture endpoint does **not** post an event today — the exporter is wired for the SDK `error` / `startup` / `shutdown` callbacks only (verified against the live `sash-safety` build, 2026-09-20). Registered here so the vocabulary is settled before the emitter lands. Planned `payload.extra`: `{capture_id, node, label, sizes}` — `capture_id` the store's sortable id, `node` the motion-graph node the frames were taken from (null when off-grid), `label` the caller's free-form name, `sizes` the per-file byte counts. `device_id` is `xarm_translocation`. Until it is emitted, the capture record itself is the audit trail: `GET /realsense/captures` on the device, and the nightly replica on the data server. |

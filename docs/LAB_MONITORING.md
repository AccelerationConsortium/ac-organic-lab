...
| `realsense_capture` | *reserved — not yet emitted* (planned: xArm exporter, on `POST /control/realsense/capture`) | The xArm's eye-in-hand D435i writes a durable capture record (colour + 16-bit depth + `meta.json`) under `C:\SDL_Data\xarm\realsense`, but the capture endpoint does **not** post an event today — the exporter is wired for the SDK `error` / `startup` / `shutdown` callbacks only (verified against the live `sash-safety` build, 2026-09-20). Registered here so the vocabulary is settled before the emitter lands. Planned `payload.extra`: `{camera_id, capture_id, node, label, sizes}` — `camera_id` the device-local camera (`rs435i` today), `capture_id` the store's sortable id, `node` the motion-graph node the frames were taken from (null when off-grid), `label` the caller's free-form name, `sizes` the per-file byte counts. `device_id` is `xarm_translocation`. Until it is emitted, the capture record itself is the audit trail: `GET /realsense/captures` on the device, and the nightly replica on the data server. |
| `graph_mode_override` / `graph_mode_restored` | *reserved — not yet emitted* (planned: xArm, on `POST /control/graph/mode` lowering the graph interlock below `strict`, and on every revert) | Registered 2026-09-20 ahead of the device's time-limited mode override (xarm-translocation `src/docs/FREEHAND_CARTESIAN_PLAN.md` Step 1) so the vocabulary is settled before the emitter lands, as with `realsense_capture`. Today `POST /control/graph/mode {mode}` is process-wide state with no expiry: a forgotten `advisory` is inherited by the next client, and the dashboard writes only the generic `control_action` row. Planned: `graph_mode_override` once per lowering with `payload.extra: {mode, reason, ttl_s, owner}` (`reason` mandatory on the device when lowering; re-issue extends the TTL and emits again), and `graph_mode_restored` once per return to `strict` with `payload.extra: {trigger}`, `trigger` ∈ `ttl_expired` \| `claim_released` \| `claim_expired` \| `disconnect` \| `explicit`. `device_id` is `xarm_translocation`. The pair is what lets the timeline answer "how long was the graph relaxed, by whom, and did it come back on its own" — the question a bare `control_action` row cannot. The fume-hood sash gate (`interlock_freehand_guard`) is unaffected by graph mode. |
| `plate_moved`, `plate_custody_mismatch`, `plate_custody_unknown` | **Run executor**, from either half: `api/app/workflow.py::custody_after_step` (since 2026-08-23) for every compiled step carrying `custody: {plate, hid, to}`, and `::custody_preflight` (since 2026-08-30) before each of those steps; **human front door** `POST /api/custody/move` (`api/app/custody.py`) for bench-top moves ([`PLATE_TRACKING.md`](PLATE_TRACKING.md) D5–D8) | **Ops audit only.** `device_id` is the equipment anchoring the destination place (or `custody` for a bench move). Every executor-written row carries **`payload.phase ∈ {preflight, after_step}`** (both since 2026-08-31) saying which half wrote it; a bench move carries no phase, since it belongs to no run. `plate_moved`: one row per recorded move, always `after_step` — `payload: {step_id, plate, hid, to, recorded, result, observed, verdict, phase, run_id, authorization_id, performed_by, source: executor\|bench}`. `plate_custody_mismatch` comes from **both** halves and the phase is what tells them apart: `preflight` = the ledger no longer puts the plate where this run's chain of accepted moves left it (`payload: {expected, actual, verdict, phase}`); `after_step` = the destination device contradicted the move just commanded. Either way a deviation Note is filed with the run and nothing auto-corrects. `plate_custody_unknown` (`after_step`): the step was sent and never answered, so **no** move was recorded and the last known place stands. Custody itself — where a plate *is* — is read from BitacoraDB (`Container.location_id` + the `ContainerAction` ledger), never from these rows; there is deliberately no `/api/history/plates` — `GET /api/custody/plates` is a read-through. |
...
- **Kuma watches services, not devices.** Uptime Kuma (dashboard host,
  ...
  PyPoe, gateways, auth, BitacoraDB) and alerts through PyPoe's

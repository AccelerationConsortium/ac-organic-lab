...
| `robot_arm` | 6 (+1 xArm-only) | `graph.{move_to,travel_to,gripper,recover_to,record,mode}` — xArm motion-graph control surface (v1.1, claim-gated); added 2026-05-31, `graph.gripper` 2026-08-13, `graph.travel_to` 2026-09-01 (device-planned multi-hop travel, UI_DESIGN §5 Step 1k). **`realsense.capture` (2026-09-20; `camera` arg since cameras became id-addressed the same day)** is attached per-device via `skills_for` to `xarm_translocation` only, not registered for the kind: the eye-in-hand D435i is hardware on one arm and the MG400 has none, so registering it for `robot_arm` would advertise a camera the device would refuse. Same reasoning that keeps UR joint control out of the shared list. See [`EQUIP_STATUS.md`](EQUIP_STATUS.md) §10 for the capture store, retention and nightly replication. |

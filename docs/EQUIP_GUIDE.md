...
| MSE viewport shows "No stream" | Viewing session refused, capacity exhausted, or authenticated broker route missing | Sign in, check `/admin/camera-streams`, and verify `/api/camera-streams/*` routes to the API; see [Camera viewing](CAMERA_VIEWING.md). Keep all legacy `/streams/*` paths blocked; never restore a raw relay proxy. |
| PTZ buttons disabled                                 | ONVIF was unreachable on last poll                              | Same as the first row                                       |
| Camera tile has no optical zoom; the zoom column only does *digital* zoom, and `zoom_in` via the API returns 409 | Expected on every Tapo dual-lens head (C245D, C246D — probed 2026-09-10): the ONVIF PTZ node advertises no zoom space, `GetStatus` has no zoom position, and pytapo has no zoom method, so the gateway reports `details.has_zoom: false`. On these models "zoom" *is* the Wide → Tele lens switch | Nothing to fix. Use the **Tele** lens tab for optical magnification and the tile's `+` / `−` column (drag to pan, double-click to toggle) to crop the view. A camera that does advertise a zoom axis gets `has_zoom: true`, optical zoom cells on the PTZ pad, and working `zoom_in` / `zoom_out` with no code change |
